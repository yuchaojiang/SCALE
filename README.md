...
* **Spike-in input?**
 ...
 SCALE uses external spike-ins to estimate the technical-noise associated paramters. SCALE does not require spike-in in every cell in the experiment. As long as some cells from the same batch have spike-ins, they can be used to estimate the dropout rates, amplification and sequencing bias during the library prep step. Make sure that the column names of spikein_input match as a subset of the column names of alleleA and alleleB matrices.
