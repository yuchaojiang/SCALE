tech_bias = function (spikein_input, alleleA, alleleB, readlength, pdf = NULL) 
{
    ...
    if (pdf == TRUE) {
        ...
        text(x = 4, y = quantile(log(Q), 0.9), paste("log(alpha) =", 
                                                     round(alpha, 3)), col = "blue")
        text(x = 4, y = quantile(log(Q), 0.8), paste("beta =", 
    ...
    if (pdf == TRUE) {
        Y.sim = seq(min(log(Y)),max(log(Y)),0.05)
        plot(Y.sim, 1 - expit(kappa + tau * Y.sim), type = "l", 
             ylim = c(0, 1), xlab = "Log(true expression)", ylab = "Percentage of zero reads", 
             col = "blue", lwd = 1.5,
             xlim=c(min(log(Y)),max(log(Y))))
        text(x = 7, y = 0.9, paste("kappa =", round(kappa, 3)), 
             col = "blue")
